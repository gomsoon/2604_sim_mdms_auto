...
This means the current repository posture is:

- `make test`
  - stable serial regression baseline
- `make test-functional`
  - separate Playwright smoke suite
- `pytest-xdist`
  - available for future hardening work
  - not yet the default regression path because repeated `n=4` validation still flakes
